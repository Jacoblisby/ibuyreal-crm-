...
export const metadata = { title: 'Assumptions — iBuyReal' };

export default async function SettingsPage() {
  if (!db) return <div className="text-sm text-slate-500">DB ikke konfigureret.</div>;
  let [row] = await db.select().from(assumptions).where(eq(assumptions.id, 'default'));
  if (!row) {
    [row] = await db.insert(assumptions).values({ id: 'default' }).returning();
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <SettingsClient assumptions={row} cases={cases} />
    </div>
  );
}
